...
export function Login({ onIn }: { onIn: () => void }) {
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    try { await api.login(pw); onIn(); } catch { setErr("Wrong password"); }
  };
  return (
    <div className="auth">
      <div className="glow" />
      <form className="auth-card" onSubmit={submit}>
        ...
        <h2>Sign in</h2>
        <input className="input" type="password" placeholder="Dashboard password" value={pw} onChange={(e) => setPw(e.target.value)} autoFocus />
