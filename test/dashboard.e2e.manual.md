...
10. On **Secrets**, click **Set value** on `httpbin · default → DEMO_KEY` → a single-use link appears. Open it in a new tab → the §5.9 auth screen (title `DEMO_KEY`, breadcrumb `httpbin · default`, no raw ref, no value echoed). Enter a value → "Sealed" confirmation. Reopen the same link → "Expired or already used." (single-use).
