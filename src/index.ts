...
async function main() {
  ...
  mountDashboard(app, {
    ...
    workspace,
    webDir,
    runQuery: (instruction, onProgress, opts) => query(queryDeps, instruction, onProgress, { commit: false, attachmentDirs: opts?.attachmentDirs, attachmentFiles: opts?.attachmentFiles, history: opts?.history }),
